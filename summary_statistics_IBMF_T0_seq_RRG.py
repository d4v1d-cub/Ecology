...
def print_summary(path_in, path_out, eps, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    ...
    fout.write("# mu sigma av_time av_div samples_div samples_multiple_eq samples_with_deaths av_m std_m nsamples\n")
    vals_list = get_all_vals(path_in, eps, N, c, avn0, tol, max_iter, damping, nseq)
    for vals in vals_list:
        mu, sigma, av_time, av_num_div, samples_div_m, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, nsamples = vals
        fout.write(f"{mu:.{ndigits}f} {sigma:.{ndigits}f} {av_time:.6f} {av_num_div:.6f} {samples_div_m} {samples_multiple_eq} {samples_with_deaths} {av_m:.6f} {std_av_m:.6f} {nsamples}\n")
    fout.close()
