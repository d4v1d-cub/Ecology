...
double new_averages(double beta, double lambda, Tnode &node, double tol_asymp, 
    ...
    if (node.W > 0){
        av_new = numerator_IBMF3(beta, lambda, node.W, node.var, node.field, integrand_av, 
                                 epsabs, epsrel, limit, workspace, params);
        q_sqr_new = numerator_IBMF3(beta, lambda, node.W, node.var, node.field, integrand_q_sqr, 
                                    epsabs, epsrel, limit, workspace, params);
        w3_new = numerator_IBMF3(beta, lambda, node.W, node.var, node.field, integrand_w3, 
                                 epsabs, epsrel, limit, workspace, params);
        den = (node.W * w3_new / 2 + q_sqr_new / node.var - node.field * av_new) / lambda;
        av_new /= den;
        q_sqr_new /= den;
        w3_new /= den;
    }else{
        if (node.var > 0){
            ...
            if (exp(-beta * hi2 / 2) / pow(hi2, beta * lambda / 2) < tol_asymp){
                ...
                q_sqr_new = node.var * numerator_q_sqr_IBMF2(beta, lambda, hi, hi2) / den;
                w3_new = av_new * av_new * av_new;
            }
        }else{
            hi = node.field;
