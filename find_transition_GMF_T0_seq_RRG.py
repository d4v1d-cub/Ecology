...
def find_all_trans(path, eps, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    fout = open(f'{path}/GMF_T0_seq_RRG_PD_Lotka_Volterra_transitions_av0_{avn0}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    
    fin = open(f'{path}/GMF_T0_seq_RRG_PD_Lotka_Volterra_summary_av0_{avn0}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'r')
    fin.readline()  # Skip header line

    lines = fin.readlines()

    transitions_m = find_transition(lines, 5)
    transitions_chi = find_transition(lines, 6)
    transitions_multiple_eq = find_transition(lines, 7)
    transitions_deaths = find_transition(lines, 8)
    ...
    for mu in mu_list:
        ...
        fout.write(f"{mu:.{ndigits}f}\t{sigma_m:.{ndigits}f}\t{sigma_below_m:.{ndigits}f}\t{sigma_chi:.{ndigits}f}\t{sigma_below_chi:.{ndigits}f}\t{sigma_multiple_eq:.{ndigits}f}\t{sigma_below_multiple_eq:.{ndigits}f}\t{sigma_deaths:.{ndigits}f}\t{sigma_below_deaths:.{ndigits}f}\n")
