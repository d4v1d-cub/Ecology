 ...
 * @param gr_inside      [out]    set to true if --gr_inside is found; if true, the interaction graph is generated by the program, otherwise it is read from the file named input_graph_name
 * @param input_graph_name [out]  string representation of the name of the file to read the interaction graph from, exactly as passed on the command line; only meaningful when gr_inside is false. Must be pre-allocated by the caller (e.g. with my_char_malloc(CHAR_LENGHT))
 * @param graph_id       [out]    identifier of the interaction graph used, exactly as passed on the command line; added to output file names so that runs on different graphs (e.g. different input_graph_name files) don't overwrite each other. Must be pre-allocated by the caller (e.g. with my_char_malloc(CHAR_LENGHT))
 * @param aji_aij        [out]    set to true if --aji-aij is found; only meaningful when gr_inside is false. If false, the interaction graph file is read with load_matrix_from_file_sparse_ij_aijaji() (graph(i,j) first, then graph(j,i)); if true, it is read with load_matrix_from_file_sparse_ij_ajiaij() (graph(j,i) first, then graph(i,j))
 */
void parse_arguments(int argc, char *argv[], int &N, double &c, char *c_label,
                      ...
                      bool &gr_inside, char *input_graph_name, char *graph_id, bool &aji_aij){
    int arg_index = 1;
    while(arg_index < argc){
        if(strcmp(argv[arg_index], "-h") == 0 || strcmp(argv[arg_index], "--help") == 0){
            ...
            fprintf(stderr, "--gr_inside  ::  if this flag is added to the arguments, the interaction graph is generated by the program; otherwise it is read from the file given with --input_graph_name\n");
            fprintf(stderr, "--input_graph_name  [string]  ::  name of the file to read the interaction graph from (only needed if --gr_inside is not set)\n");
            fprintf(stderr, "--graph_id  [string]  ::  identifier of the interaction graph used, added to output file names so that runs on different graphs don't overwrite each other\n");
            fprintf(stderr, "--aji-aij  ::  if this flag is added to the arguments, the interaction graph file (only needed if --gr_inside is not set) is read as i, j, graph(j,i), graph(i,j) per line, instead of the default i, j, graph(i,j), graph(j,i)\n");
            exit(MY_SUCCESS);
            ...
            arg_index++;
        }else if(strcmp(argv[arg_index], "--gr_inside") == 0){
            gr_inside = true;
            arg_index++;
        }else if(strcmp(argv[arg_index], "--input_graph_name") == 0){
            arg_index++;
            snprintf(input_graph_name, CHAR_LENGHT, "%s", argv[arg_index]);
            arg_index++;
        }else if(strcmp(argv[arg_index], "--graph_id") == 0){
            arg_index++;
            snprintf(graph_id, CHAR_LENGHT, "%s", argv[arg_index]);
            arg_index++;
        }else if(strcmp(argv[arg_index], "--aji-aij") == 0){
            aji_aij = true;
            arg_index++;
        }else{
 ...
 * @param gr_inside      whether the interaction graph is generated by the program (true) or read from input_graph_name (false)
 * @param input_graph_name name of the file the interaction graph is read from, when gr_inside is false
 * @param graph_id       identifier of the interaction graph used, as used to build output file names
 * @param aji_aij        whether the interaction graph file is read as i, j, graph(j,i), graph(i,j) (true) or i, j, graph(i,j), graph(j,i) (false); only meaningful when gr_inside is false
 */
void print_params(int N, double c, char *c_label, double mu, char *mu_label,
    ...
    fprintf(stderr, "Generate the interaction graph inside the program (gr_inside): %s\n", BOOL_STR(gr_inside));
    if(not gr_inside){
        fprintf(stderr, "Name of the file to read the interaction graph from (input_graph_name): %s\n", input_graph_name);
        fprintf(stderr, "Read the interaction graph file as i, j, graph(j,i), graph(i,j) (aji_aij): %s\n", BOOL_STR(aji_aij));
    }
    fprintf(stderr, "Identifier of the interaction graph used (graph_id): %s\n", graph_id);
    fprintf(stderr, " #######################\n\n");
    return;
}
