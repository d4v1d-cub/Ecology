...
def get_all_vals(path, eps, N, c, avn0, tol, max_iter, damping, nseq):
    ...
    for filename, mu, sigma in sorted_data:
        ...
        if found:
            vals_list.append((mu, sigma, av_time, av_num_div_m, av_num_div_chi, samples_div_m, samples_div_chi, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, av_chi, std_av_chi))
...
def print_summary(path_in, path_out, eps, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    ...
    fout.write("# mu sigma av_time av_div_m av_div_chi samples_div_m samples_div_chi samples_multiple_eq samples_with_deaths av_m std_m av_chi std_chi\n")
