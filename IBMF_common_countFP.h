...
double average_FP(long N, Tnode *nodes, int nFP){
    double av = 0;
    for (long i = 0; i < N; i++){
        av += nodes[i].fixed_points[nFP];
    }
    return av / N;
}
...
double average_sqr_FP(long N, Tnode *nodes, int nFP){
    double av_sqr = 0;
    for (long i = 0; i < N; i++){
        av_sqr += nodes[i].fixed_points[nFP]* nodes[i].fixed_points[nFP];
    }
    return av_sqr / N;
}
...
// Funzione per confrontare un punto fisso con tutti quelli memorizzati
int find_fixed_point_index(Tnode *nodes, long N, double tol_fixed_point) {
    if (nodes[0].fixed_points.empty()) {
        return -1; // Nessun punto fisso memorizzato
    }
