...
def find_all_trans(path, eps, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    fout_div = open(f'{path}/IBMF_T0_seq_RRG_PD_Lotka_Volterra_transitions_div_av0_{avn0}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    fout_mult = open(f'{path}/IBMF_T0_seq_RRG_PD_Lotka_Volterra_transitions_mult_av0_{avn0}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
