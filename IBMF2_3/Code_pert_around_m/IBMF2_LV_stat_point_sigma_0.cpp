...
double new_averages(double beta, double lambda, Tnode &node, double hmax, 
                    double **coefficients, int iter, double tol, 
                    double normfactor = 1e-14){
    double delta_av, delta_beta_qm, Q, hi, hi_div_Q, den, av_new, q_sqr_new, beta_qm_new;
    ...
    delta_beta_qm = fabs(beta_qm_new - node.beta_qm);

    if (!node.beta_qm_converged && delta_beta_qm < tol){
    ...
    return max(delta_av, delta_beta_qm);
}
...
int main(int argc, char *argv[]) {
    ...
    for (double mu = mu0; mu < muf + dmu / 2; mu += dmu) {
        ...
        iter = convergence(beta, lambda, c, mu, node, tol, max_iter, divergence, 
                           hmax, coefficients);
        if (divergence){
            cout << mu << "\t" << iter << "\t" << "diverges" << "\t" << node.av << "\t" << node.beta_qm << "\t" << node.beta_qm_converged << endl;
        }else{
            conv = iter < max_iter;
            cout << mu << "\t" << iter << "\t" << conv << "\t" << node.av << "\t" << node.beta_qm << "\t" << node.beta_qm_converged << endl;
        }
    }
