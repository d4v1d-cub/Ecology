...
def find_all_trans(path, T, lda, eps, N, c, avn0, chi0, dn, dchi, ninitconds, tol, max_iter, damping, nseq, ndigits):
    fout_div = open(f'{path}/GMF_seq_RRG_T_{T}_lambda_{lda}_PD_Lotka_Volterra_transitions_div_av0_{avn0}_chi0_{chi0}_dn_{dn}_dchi_{dchi}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    fout_mult = open(f'{path}/GMF_seq_RRG_T_{T}_lambda_{lda}_PD_Lotka_Volterra_transitions_mult_av0_{avn0}_chi0_{chi0}_dn_{dn}_dchi_{dchi}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    fout_deaths = open(f'{path}/GMF_seq_RRG_T_{T}_lambda_{lda}_PD_Lotka_Volterra_transitions_deaths_av0_{avn0}_chi0_{chi0}_dn_{dn}_dchi_{dchi}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')

    fin = open(f'{path}/GMF_seq_RRG_T_{T}_lambda_{lda}_PD_Lotka_Volterra_summary_av0_{avn0}_chi0_{chi0}_dn_{dn}_dchi_{dchi}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'r')
...
def main():
    ...
    path = "/media/david/Seagate Expansion Drive/Salva/Salva_Data_Investigacion/Grupo_de_investigacion/Ecology/Results/GMF/"
    # path = "/mnt/d/Research/Ecology/Results/GMF/"


    ndigits = 3

    for N in N_list:
        find_all_trans(path, T, lda, eps, N, c, avn0, chi0, dn, dchi, ninitconds, tol, max_iter, damping, nseq, ndigits)
