...
def get_all_vals(path, lda, eps, sigma, N, c, avn0, tol, max_iter, damping, nseq):
    ...
    for filename, T, mu in sorted_data:
        ...
        if found:
            vals_list.append((T, mu, av_time, av_num_div_m, av_num_div_chi, samples_div_m, samples_div_chi, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, av_chi, std_av_chi, nsamples))
...
def print_summary(path_in, path_out, lda, eps, sigma, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    ...
    fout.write("# T mu av_time av_div_m av_div_chi samples_div_m samples_div_chi samples_multiple_eq samples_with_deaths av_m std_m av_chi std_chi nsamples\n")
    vals_list = get_all_vals(path_in, lda, eps, sigma, N, c, avn0, tol, max_iter, damping, nseq)
    for vals in vals_list:
        T, mu, av_time, av_num_div_m, av_num_div_chi, samples_div_m, samples_div_chi, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, av_chi, std_av_chi, nsamples = vals
        fout.write(f"{T:.{ndigits}f} {mu:.{ndigits}f} {av_time:.6f} {av_num_div_m:.6f} {av_num_div_chi:.6f} {samples_div_m} {samples_div_chi} {samples_multiple_eq} {samples_with_deaths} {av_m:.6f} {std_av_m:.6f} {av_chi:.6f} {std_av_chi:.6f} {nsamples}\n")
    fout.close()
