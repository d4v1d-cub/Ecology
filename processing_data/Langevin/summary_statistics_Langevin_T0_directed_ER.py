...
def summary_statistics(path, filename):
    ...
    if len(all_lines) > 0:
        ...
        return av_time, samples_div, samples_multiple_eq, runtime_conv, std_runtime_conv, nsamples, True
    else:
...
def get_all_vals(path, eps, lda, tol_fixed_point, T):
    ...
    for filename, N, c, mu, sigma in sorted_data:
        av_time, samples_div, samples_multiple_eq, runtime_conv, std_runtime_conv, nsamples, found = summary_statistics(path, filename)
        if found:
            vals_list.append((N, c, mu, sigma, av_time, samples_div, samples_multiple_eq, runtime_conv, std_runtime_conv, nsamples))
...
def print_summary(path_in, path_out, eps, lda, tol_fixed_point, T, ndigits):
    ...
    fout.write("#N  c  mu  sigma  av_time  samples_div  samples_mult_eq  prob_div  error_prob  runtime(s)  std_runtime(s)  nsamples\n")
    vals_list = get_all_vals(path_in, eps, lda, tol_fixed_point, T)
    for vals in vals_list:
        N, c, mu, sigma, av_time, samples_div, samples_multiple_eq, runtime_conv, std_runtime_conv, nsamples = vals
        ...
        fout.write(f"{N} {int(c * 10 ** ndigits)} {int(mu * 10 ** ndigits)} {int(sigma * 10 ** ndigits)} {av_time:.6f} {samples_div} {samples_multiple_eq} {prob:.6f} {error:.6f} {runtime_conv} {std_runtime_conv} {nsamples}\n")
    fout.close()
