...
def filter_files(path, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq):
    files_names = []

    # Define the pattern for matching filenames
    pattern = f'IBMF_seq_RRG_T_*_lambda_{lda}_PD_Lotka_Volterra_final_av0_{avn0}_dn_{dn}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_mu_*_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt'

    # Iterate through the files in the specified directory
    for filename in os.listdir(path):
        if fnmatch.fnmatch(filename, pattern):
            parts = filename.split('_')
            files_names.append((filename, float(parts[4]), float(parts[24])))
    sorted_pairs = sorted(files_names, key=lambda x: (x[1], x[2]))  # Sort by mu value
    return sorted_pairs
...
def get_all_vals(path, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq):
...
def print_summary(path_in, path_out, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq, ndigits):
    fout = open(f'{path_out}/IBMF_seq_RRG_PD_Lotka_Volterra_summary_av0_{avn0}_lambda_{lda}_dn_{dn}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    ...
    vals_list = get_all_vals(path_in, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq)
...
def main():
    ...
    path_in = "/media/david/Seagate Expansion Drive/Salva/Salva_Data_Investigacion/Grupo_de_investigacion/Ecology/Results/IBMF/AllData/PhaseDiagram/sigma0/"
    path_out = "/media/david/Seagate Expansion Drive/Salva/Salva_Data_Investigacion/Grupo_de_investigacion/Ecology/Results/IBMF/"

    ndigits = 3

    for N in N_list:
        print_summary(path_in, path_out, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq, ndigits)
    return 0
