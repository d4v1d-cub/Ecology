...
def print_params_search_below_line(path_in, filein, path_out, fileout, dpar_trans, pos_par_fixed, 
                 pos_par_trans_1, pos_par_trans_2, shift_above, par_fixed_list, eps, seed_block, nsampl_each, 
    ...
    with open(f'{path_out}/{fileout}', 'a') as fo:
        for par_fixed in par_fixed_list:
            if par_fixed in par_in_trans:
                ...
                min_val = 0
                max_val = max(trans_above_max, trans_below_max) + shift_above
                par_list = np.arange(min_val, max_val + dpar_trans / 2, dpar_trans)
                for par in par_list:
                    if (par_fixed, par) not in already_printed:
                        fo.write(f"{eps} {par_fixed:.3f} {par:.3f} {seed_block} {nsampl_each}\n")
...
def print_params_missing(path_in_trans, path_in_already, filein_trans, filein_already, path_out, fileout, 
                         dpar_trans, pos_par_fixed, pos_par_trans_1, pos_par_trans_2, shift_below, shift_above, 
                         par_fixed_list, eps, seed_block, nsampl_each):
    transitions = read_transitions(f'{path_in_trans}/{filein_trans}', pos_par_fixed, 
                                   pos_par_trans_1, pos_par_trans_2)
    already_done = read_transitions(f'{path_in_already}/{filein_already}', pos_par_fixed, 
                                    pos_par_trans_1, pos_par_trans_2)
    counter = 0
    par_in_trans = transitions.keys()
    with open(f'{path_out}/{fileout}', 'w') as fo:
        for par_fixed in par_fixed_list:
            if par_fixed not in already_done:
                if par_fixed in par_in_trans:
                    trans_min, trans_max = transitions[par_fixed]
                    min_val = max(0, trans_min - shift_below)
                    max_val = trans_max + shift_above
...
def main():
    ...
    for N in N_list:
        filein = f'IBMF_seq_RRG_T_0.000_lambda_0.000_PD_Lotka_Volterra_transitions_mult_av0_0.5_dn_0.5_ninitconds_10_tol_1e-6_maxiter_10000_eps_0.000_N_{N}_c_3_damping_0.2_nseq_1.txt'
        ...
        print_params_search_below_line(path_in, filein, path_out, fileout, dpar_trans, pos_par_fixed, 
                                       pos_par_trans_1, pos_par_trans_2, shift_above, 
                                       par_fixed_list, eps, seed_block, nsampl_each)


    N_list = [8192, 16384, 32768]

    filein = f'IBMF_seq_RRG_T_0.000_lambda_0.000_PD_Lotka_Volterra_transitions_mult_av0_0.5_dn_0.5_ninitconds_10_tol_1e-6_maxiter_10000_eps_0.000_N_4096_c_3_damping_0.2_nseq_1.txt'
    ...
    # IBMF
    # path_in = "/media/david/Data/UH/Grupo_de_investigacion/Ecology/Results/IBMF/"
    # path_in_already = "/media/david/Seagate Expansion Drive/Salva/Salva_Data_Investigacion/Grupo_de_investigacion/Ecology/Results/IBMF"
    # path_in_trans = "/media/david/Seagate Expansion Drive/Salva/Salva_Data_Investigacion/Grupo_de_investigacion/Ecology/Langevin/Results"
    # N_list = [128, 256, 512, 1024, 2048, 4096]
    # dpar_fixed = 0.003
    # par_fixed_start = 0.000
    # par_fixed_end = 0.351
    ...
    # for N in N_list:
    #     filein_already = f'IBMF_seq_RRG_T_0.000_lambda_0.000_PD_Lotka_Volterra_transitions_mult_av0_0.5_dn_0.5_ninitconds_10_tol_1e-6_maxiter_10000_eps_0.000_N_{N}_c_3_damping_0.2_nseq_1.txt'
    #     filein_trans = f'Lotka-Volterra_transition_mult_epsilon_0.0_Partially_AsymGauss_lambda_1e-06_tol_1e-08_N_{N}_c_3.00_T_0.0.txt'
    ...
    #     print_params_missing(path_in_trans, path_in_already, filein_trans, filein_already, path_out, 
    #                          fileout, dpar_trans, pos_par_fixed, 
    #                          pos_par_trans_1, pos_par_trans_2, shift_below, shift_above, 
    #                          par_fixed_list, eps, seed_block, nsampl_each)
