...
def find_all_trans(path, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq, ndigits):
    fout_div = open(f'{path}/IBMF_seq_RRG_PD_Lotka_Volterra_transitions_div_av0_{avn0}_lambda_{lda}_dn_{dn}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    fout_mult = open(f'{path}/IBMF_seq_RRG_PD_Lotka_Volterra_transitions_mult_av0_{avn0}_lambda_{lda}_dn_{dn}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    
    fin = open(f'{path}/IBMF_seq_RRG_PD_Lotka_Volterra_summary_av0_{avn0}_lambda_{lda}_dn_{dn}_ninitconds_{ninitconds}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'r')
...
def main():
    ...
    for N in N_list:
        find_all_trans(path, lda, eps, sigma, N, c, avn0, dn, ninitconds, tol, max_iter, damping, nseq, ndigits)
