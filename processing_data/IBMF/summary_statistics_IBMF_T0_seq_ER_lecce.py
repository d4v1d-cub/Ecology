...
def summary_statistics(path, filename):
    ...
    if len(all_lines) > 0:
        ...
        return av_time, av_num_div, samples_div_m, av_m, std_av_m, \
...
def get_all_vals(path, T, lda, eps, avn0, dn, ninitconds, tol, max_iter, damping, nseq):
    ...
    for filename, N, c, mu, sigma in sorted_data:
        av_time, av_num_div, samples_div_m, av_m, std_av_m, runtime_conv, std_runtime_conv, nsamples, found = summary_statistics(path, filename)
        if found:
            vals_list.append((N, c, mu, sigma, av_time, av_num_div, samples_div_m, av_m, std_av_m, runtime_conv, std_runtime_conv, nsamples))
...
def print_summary(path_in, path_out, T, lda, eps, avn0, dn, ninitconds, tol, max_iter, damping, nseq, ndigits):
    ...
    for vals in vals_list:
        N, c, mu, sigma, av_time, av_num_div, samples_div_m, av_m, std_av_m, runtime_conv, std_runtime_conv, nsamples = vals
        ...
        fout.write(f'{N} {int(c * 10 ** ndigits)} {int(mu * 10 ** ndigits)} {int(sigma * 10 ** ndigits)} {av_time:.{6}f} {av_num_div:.{6}f} {samples_div_m} {samples_div_m / nsamples:.{6}f} {error:.{6}f} {av_m:.{6}f} {std_av_m:.{6}f} {runtime_conv} {std_runtime_conv} {nsamples}\n')
    fout.close()
