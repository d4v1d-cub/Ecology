...
def filter_files(path, lda, eps, sigma, N, c, avn0, tol, max_iter, damping, nseq):
    ...
    pattern = f'GMF_seq_RRG_PD_Lotka_Volterra_final_av0_{avn0}_T_*_lambda_{lda}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_mu_*_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt'
...
def summary_statistics(path, filename):
    ...
    if len(all_lines) > 0:
        ...
        return av_time, av_num_div_m, av_num_div_chi, samples_div_m, samples_div_chi, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, av_chi, std_av_chi, True
    else:
...
def get_all_vals(path, lda, eps, sigma, N, c, avn0, tol, max_iter, damping, nseq):
    ...
    for filename, T, mu in sorted_data:
        av_time, av_num_div_m, av_num_div_chi, samples_div_m, samples_div_chi, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, av_chi, std_av_chi, found = summary_statistics(path, filename)
        if found:
            vals_list.append((T, mu, av_time, av_num_div_m, av_num_div_chi, samples_div_m, samples_div_chi, samples_multiple_eq, samples_with_deaths, av_m, std_av_m, av_chi, std_av_chi))
        print(f'Processed  T={T}   sigma={mu}')
    return vals_list
...
def print_summary(path_in, path_out, lda, eps, sigma, N, c, avn0, tol, max_iter, ndigits, damping, nseq):
    fout = open(f'{path_out}/GMF_seq_RRG_PD_Lotka_Volterra_summary_av0_{avn0}_lambda_{lda}_tol_{tol}_maxiter_{max_iter}_eps_{eps}_sigma_{sigma}_N_{N}_c_{c}_damping_{damping}_nseq_{nseq}.txt', 'w')
    fout.write("# T mu av_time av_div_m av_div_chi samples_div_m samples_div_chi samples_multiple_eq samples_with_deaths av_m std_m av_chi std_chi\n")
    vals_list = get_all_vals(path_in, lda, eps, sigma, N, c, avn0, tol, max_iter, damping, nseq)
    for vals in vals_list:
        fout.write(f"{vals[0]:.{ndigits}f} {vals[1]:.{ndigits}f} {vals[2]:.6f} {vals[3]:.6f} {vals[4]:.6f} {vals[5]} {vals[6]} {vals[7]} {vals[8]} {vals[9]:.6f} {vals[10]:.6f} {vals[11]:.6f} {vals[12]:.6f}\n")
    fout.close()
...
def main():
    ...
    path_in = "/media/david/Data/UH/Grupo_de_investigacion/Ecology/Results/GMF/AllData/PhaseDiagram/sigma0/"
