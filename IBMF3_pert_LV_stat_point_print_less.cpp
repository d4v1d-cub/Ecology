...
double numerator_av_IBMF(double beta, double lambda, double hi, double hi2){
...
double numerator_q_sqr_IBMF2(double beta, double lambda, double hi, double hi2){
...
double denominator_IBMF(double beta, double lambda, double hi, double hi2){
...
double integrand_av(double ni, void *params){
    ...
    double M = params_d[4];
    double integrand_av = pow(ni, beta * lambda) * exp(-beta * (W * ni * ni * ni / 6 + ni * ni / 2 / S2 - M * ni));
    return integrand_av;
}
...
double numerator_IBMF3(double beta, double lambda, double W, double S2, double M, 
...
double new_averages(long N, double beta, double lambda, Tnode *nodes, double tol_asymp, 
    ...
    double delta = 0, delta_av, delta_q_sqr, delta_w3, s, hi, hi2, den, av_new, q_sqr_new, w3_new;
    
    for (long i = 0; i < N; i++){
        if (nodes[i].W > 0){
            av_new = numerator_IBMF3(beta, lambda, nodes[i].W, nodes[i].var, nodes[i].field, integrand_av, 
                                     epsabs, epsrel, limit, workspace, params);
            q_sqr_new = numerator_IBMF3(beta, lambda, nodes[i].W, nodes[i].var, nodes[i].field, integrand_q_sqr, 
                                        epsabs, epsrel, limit, workspace, params);
            w3_new = numerator_IBMF3(beta, lambda, nodes[i].W, nodes[i].var, nodes[i].field, integrand_w3, 
                                     epsabs, epsrel, limit, workspace, params);
            den = (nodes[i].W * w3_new / 2 + q_sqr_new / nodes[i].var - nodes[i].field * av_new) / lambda;
            av_new /= den;
            q_sqr_new /= den;
            w3_new /= den;
        }else{
            if (nodes[i].var > 0){
                ...
                if (exp(-beta * hi2 / 2) / pow(hi2, beta * lambda / 2) < tol_asymp){
                    hi = nodes[i].field;
                    hi2 = hi * hi;
                    den = denominator_IBMF(beta, lambda, hi, hi2);
                    av_new = numerator_av_IBMF(beta, lambda, hi, hi2) / den;
                    ...
                    hi = s * nodes[i].field;
                    den = denominator_IBMF(beta, lambda, hi, hi2);
                    av_new = s * numerator_av_IBMF(beta, lambda, hi, hi2) / den; 
                    q_sqr_new = nodes[i].var * numerator_q_sqr_IBMF2(beta, lambda, hi, hi2) / den;
                    w3_new = av_new * av_new * av_new;
                }
            }else{
                hi = nodes[i].field;
